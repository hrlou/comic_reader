...
use crate::{app, prelude::*, utils::{save_image_to_file, save_page_image}};

pub fn handle_input(app: &mut CBZViewerApp, ctx: &Context) {
    // Keyboard navigation
    ...
    if app.on_goto_page {
        app.on_goto_page = false;
        let page: usize = app.page_goto_box.parse().unwrap_or(1);
        if app.goto_page(page - 1) {
            app.ui_logger.info(format!("Navigated to page {}", page));
        } else {
            app.ui_logger
...
pub fn ui_pages_module(app: &mut CBZViewerApp, ui: &mut Ui, total_pages: usize) {
...
pub fn ui_logger_module(app: &mut CBZViewerApp, ui: &mut Ui) {
...
pub fn ui_file_dropdown_module(app: &mut CBZViewerApp, ui: &mut Ui) {
    egui::menu::bar(ui, |ui| {
        ui.menu_button("File", |ui| {
            ...
            if ui.button("Save Page...").clicked() {
                // save_image_to_file(app.image_lru, path);]
                save_page_image(app, app.current_page);
            }
        });
    });
}

pub fn ui_reading_options_module(app: &mut CBZViewerApp, ui: &mut Ui, total_pages: usize) {
...
pub fn draw_top_bar(app: &mut CBZViewerApp, ctx: &Context, total_pages: usize) {
    egui::TopBottomPanel::top("top_bar").show(ctx, |ui| {
        ui.horizontal(|ui| {
            ui_file_dropdown_module(app, ui);
            ui.with_layout(Layout::right_to_left(egui::Align::Center), |ui| {
                ui_reading_options_module(app, ui, total_pages);
            });
        });
    });
}
...
pub fn draw_bottom_bar(app: &mut CBZViewerApp, ctx: &Context, total_pages: usize) {
    egui::TopBottomPanel::bottom("bottom_bar").show(ctx, |ui| {
        ui.horizontal(|ui| {
            ui_zoom_slider_module(app, ui); 
            ui.separator();
            ui_goto_page_module(app, ui);
            ui.separator();
            ui.with_layout(Layout::right_to_left(egui::Align::Center), |ui| {
                ui_pages_module(app, ui, total_pages);
                ui_logger_module(app, ui);
            });
        });
    });
}
